...
CHANNELS_FILE_PATH = os.getenv('CHANNELS_FILE_PATH')
...
config_data = load_config(CHANNELS_FILE_PATH)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(f"Received message with ID: {message_id} in chat ID: {chat_id}")

    
    for msg in messages_config["chat_messages"]:
        reply_image_path = msg.get("reply_image_path")
        if reply_image_path:
            await context.bot.send_photo(chat_id=chat_id, photo=reply_image_path, caption=f"{msg['reply_text']}", reply_to_message_id=message_id)
            logger.info(f"Photo reply with caption was sent to chat {chat_id}")
        else: 
            await context.bot.send_message(chat_id=chat_id, text=f"{msg['reply_text']}", reply_to_message_id=message_id)
